 ...
 */
  ...
  static async haveSession(req, res, next) {
    ...
    || req.path === '/topics' || req.baseUrl === '/topics' || req.baseUrl === '/topics') {
      ...
      if (req.headers.authorization === undefined) {
        newResponse.createResponse(`You need to log in or sign up1 ${req.params('threadId')}  ${req.params('topicId')}  ${req.originalUrl)} ${req.params('postId')} ${req.path} `, 401, '/users', 'POST');
        newResponse.response.message = newResponse.createMessage();
        next(res.status(newResponse.response.status).send(newResponse.response));
      } else {
